# Deep-Dive: JPA vs. Hibernate vs. Spring Data JPA
...
1. **JDBC (Java Database Connectivity)**: The foundational API. You write raw SQL strings, manually open/close connections, and loop through `ResultSet` to map columns to Java fields.
   * *Problem*: High boilerplate, risk of SQL injection, manually handling resource leaks, database-specific SQL (vendor lock-in).
2. **Hibernate (The ORM Pioneer)**: Created to solve JDBC boilerplate. It maps Java objects directly to tables (Object-Relational Mapping).
   * *Problem*: It was a proprietary library. If you wrote Hibernate-specific code and wanted to switch to another ORM tool (like TopLink/EclipseLink), you had to rewrite your codebase.
3. **JPA (Jakarta/Java Persistence API)**: Introduced to standardize ORM in Java. It is a **specification** (a set of interfaces and annotations), not a working product.
   * *Benefit*: Standardized ORM. Hibernate rewritten itself to implement JPA. Developers now program against JPA interfaces, keeping their code vendor-neutral.
4. **Spring Data JPA**: Developed by Spring to sit on top of JPA. Even with JPA, you still had to write DAO classes with boilerplate `EntityManager` operations for simple CRUD. Spring Data JPA eliminates this by dynamically generating implementation classes from interfaces at runtime.
...
| **Hibernate** | **USB Cable** | A physical cable manufactured by a company (like Anker or Belkin). It implements the USB specification so you can actually transfer data. |
...
* **EntityManager**: The primary interface used to interact with the persistence context (perform CRUD, query execution).
* **EntityTransaction**: Manages database transaction boundaries (begin, commit, rollback).
...
Understanding how JPA tracks entities is critical:
```
             [ New / Transient ]
                      │  persist()
                      ▼
 ...
 │               [ Managed ]               │ ◄───┐
 └────────────────────┬────────────────────┘     │
   remove() │         │ detach() / close()       │ merge()
            ▼         ▼                          │
       [ Removed ]   [ Detached ] ───────────────┘
```
1. **New (Transient)**: The object is created in memory but is not associated with an `EntityManager` and has no row in the database.
2. **Managed (Persistent)**: Associated with the current `EntityManager` session. Any changes to this object's fields will be automatically detected and saved to the database upon transaction commit (**Dirty Checking**).
3. **Detached**: The session is closed, or the entity is explicitly detached. The entity exists in memory and the database, but changes to it are no longer tracked.
4. **Removed**: Marked for deletion. It will be deleted from the database when the transaction commits.
...
### Visualizing JPA Interfaces vs. Hibernate Implementations
```
┌───────────────────────────────────────┐
│           JPA Specification           │
│   (jakarta.persistence)               │
...
└──────────────────▲────────────────────┘
                   │
    implemented by │ (extends interfaces under the hood)
                   │
┌──────────────────┴────────────────────┐
...
* **The Solution**:
  * Fetch relationships eagerly *only* if they are always needed.
